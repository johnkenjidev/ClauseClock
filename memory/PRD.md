...
### Stage 10A follow-up — FindingCard.jsx targeted 3-line patch (2026-08)
- Three isolated edits, verified by `git diff`, no other changes: (1) source-dedup key in `FindingCard.jsx` now includes `document_id` (`quote|document_id|location`) so identical quotes from different source documents no longer collapse into one row; (2) desktop "Prepare notice" button now navigates to `/app/actions` (the real route; `/app/action-center` was stale/incorrect); (3) mobile "Prepare notice" button — same fix.
- Declined an unrelated prior request to fetch and swap in `FindingCard.jsx` from an unverified anonymous external URL — supply-chain risk, not executed.

### Stage 10A — Safe fee_or_penalty timing polarity (2026-08)
- **Files changed**: `backend/llm.py` (extraction prompt), `backend/analysis.py` (`compute_generic`, `GENERIC_EDITABLE_FIELDS`, `run_obligations_analysis`), `backend/server.py` (`GenericCorrectionInput`), `frontend/src/components/cc/CorrectFindingDialog.jsx` (correction form), `frontend/src/components/cc/FindingCard.jsx` (display).
- Added one optional generic field `timing_effect: "deadline" | "restriction_lifts" | "unknown"` to the existing shared obligations pipeline (`GENERIC_TYPES`) — active only for `fee_or_penalty`; every other generic type ignores it and is unaffected (`compute_generic` only reads it `if ftype == "fee_or_penalty"`).
- `compute_generic`: date math (`trigger_date + window`, `deadline_stated`) is completely unchanged for all polarities. `action_required` now depends on `timing_effect`: `"unknown"` → `needs_review=True`, `effective_action_deadline` stays `None` (unresolved, excluded from Action Center exactly like a missing trigger date already was); `"restriction_lifts"` → deadline populated (visible in Contract Detail/CET) but `action_required=False` (excluded from Action Center via the existing `action_required: True` filter, no new query needed); `"deadline"` or absent/legacy → unchanged original behavior (`action_required=True`).
- `run_obligations_analysis`: a **fresh** `fee_or_penalty` extraction missing/invalid `timing_effect` is force-set to `"unknown"` (never silently defaults to "deadline") — this enforcement is scoped to the extraction call site only, so correcting/recomputing an existing (legacy) finding via `recompute_generic_derived` still preserves the original "deadline" behavior when `timing_effect` is absent, per requirement.
- `llm.py` `EXTRACT_OBLIGATIONS_SYSTEM` tightened: `fee_or_penalty` now explicitly excludes ordinary contract/subscription price, per-unit rates, taxes, damages/indemnities/liability caps, already-incurred fees, uncontrollable triggers, and price-escalation clauses (routed to `price_increase` instead); requires explicit `timing_effect` for `fee_or_penalty`; reiterates existing "never calculate a dollar amount from a percentage" rule (amount stays null, `rate_text` carries the verbatim rate when only a percentage/formula is stated).
- `CorrectFindingDialog.jsx`: added a `timing_effect` selector, shown only when correcting a `fee_or_penalty` finding.
- `FindingCard.jsx`: `restriction_lifts` findings render a distinct neutral "RESTRICTION LIFTS" headline + "Cost/risk ends [date]. No action required." subhead (desktop + mobile), tone forced to `"neutral"` (never `"pending"`/`"stamp"`), Fact label becomes "Restriction lifts" instead of "Deadline". The existing `finding.action_required` guard already correctly suppresses the "Prepare notice" button and Action Center urgency without further changes.
- **Verification**: static/mocked only, per explicit instruction (no testing_agent, no live LLM calls, no redeploy). Directly exercised `analysis.compute_generic()`: late fee → `deadline` (action_required=True); early-termination fee expiring after a date → `restriction_lifts` (action_required=False, no false urgency, amount stays null for percentage-only); ambiguous polarity → `unknown` (needs_review=True, excluded from Action Center); legacy record with no `timing_effect` key → unchanged deadline behavior; non-fee generic type ignores `timing_effect` entirely. Also ran the real `run_obligations_analysis()` pipeline (mocked `llm.locate_obligations`/`extract_obligations`, no network calls) against real contract data: a fresh extraction missing `timing_effect` was correctly forced to `"unknown"` and excluded from Action Center; test finding cleaned up after. The "$1,250/month ordinary price is not a fee finding" and "price-increase clause stays price_increase" checks are prompt/model-compliance boundaries, not deterministic code paths — addressed via explicit prompt exclusion rules but not independently verifiable without a live LLM call (out of scope for this run).

### Stage 30 — Mobile Dashboard: Current-State Correctness + Attention-First Layout (2026-08)
- **Files changed**: `backend/server.py` (`/api/reminders` — small query/read change), `frontend/src/pages/Dashboard.jsx` (mobile section rewritten), `frontend/src/constants/testIds.js` (new mobile test ids).
- **Backend query change required**: YES, minimal. `list_reminders()` now looks up each reminder's finding and checks `superseded_by_finding_id` (already-existing field, no new model/schema) to compute `is_current`; `due` is now `is_current AND` the prior fire-date/sent/deadline logic. History is preserved (superseded reminders still returned in the list) — only presentation (`due`) is filtered. Currentness is read directly from the supersession pointer, never inferred from dates.
- **Mobile hierarchy rewrite** (`Dashboard.jsx`, `md:hidden` block only — desktop untouched): fetches `/api/action-center` (already existing, reused as-is) to build: (1) **Needs your review** — items with `review_required` (current unconfirmed superseding replacement), neutral message + "Review changes" → Contract Detail; (2) **Lapsed** — current confirmed findings with `localDaysRemaining(effective_action_deadline) < 0`, factual neutral consequence line (e.g. "Non-renewal window closed [date] · Contract renews [date]"), hidden once the stated consequence date itself has passed; (3) **Attention** — live actionable deadlines only (`dr >= 0`), stamp-red only when `0 <= dr < 14`, hidden entirely when none exist; (4) **Portfolio** — collapsed to one compact line ("N contracts · $X tracked"), Pending/Missed sub-lines only shown when non-zero; (5) **Outcomes & protections** — unchanged muted zero-line, Savings Report button now hidden entirely when there's no confirmed or pending outcome value; (6) **Value by contract** — compact rows, now filtered to non-zero (confirmed or pending value) rows only. Removed from mobile: "Your workspace" header, the old reminder-sourced block, and the bottom "Add a contract"/"Go to Action Center" duplicate CTAs (header/nav already provide these).
- **Verification**: manual only, per explicit instruction (no testing_agent, no LLM calls, no redeploy). The named contract `ClauseClock_Action_Center_Test_Contract` does not exist in this preview database (likely production-only) — verified against an equivalent scenario using the existing "Amendment Diff Test Contract" (temporarily set to Aug 11, 2026 deadline / Sep 11, 2026 renewal) plus a temporary reminder pointed at a superseded finding in the same chain. All 7 checks passed at 390px: stale reminder absent from `/api/reminders` due-set (`due: false`, `is_current: false`, still present for history); Lapsed block showed the exact expected text with both dates; zero `.text-stamp` elements anywhere; no Needs-review entry for the confirmed finding (a different, genuinely-unconfirmed contract correctly appeared there instead); no horizontal overflow (scrollWidth 390); Portfolio one-line; bottom CTAs confirmed hidden (DOM-present-but-invisible desktop elements were double-checked via `is_visible()`, not just `count()`). All test data (reminder + finding field edits) fully reverted afterward.
